...
COMBINED_FOLDER = "metadata_model/db_all_queries"   # combined view files land here
...
def write_combined_views(dst_dir: Path, combined_dir: Path):
    """
    For every immediate sub-folder inside dst_dir, collects all converted
    .sql files that do NOT contain a CREATE TABLE statement, sorts them by
    filename, and writes them into a single file named <subfolder>_views.sql
    inside combined_dir (e.g. db_all_queries/template1_views.sql).

    This combined file can be pasted directly into the BigQuery console or
    run with `bq query` to deploy all views in one shot.
    """
    ...
    for sub in sorted(dst_dir.iterdir()):
        ...
        view_files = sorted(
            f for f in sub.glob("*.sql")
            if not _TABLE_DDL_RE.search(f.read_text(encoding='utf-8'))
        )

        if not view_files:
            continue

        combined_path = combined_dir / f"{sub.name}_views.sql"
        with open(combined_path, 'w', encoding='utf-8') as out:
            out.write(f"-- {'='*56}\n")
            out.write(f"-- COMBINED VIEWS — {sub.name}\n")
            out.write(f"-- Generated by snowflake_to_bigquery_migrator\n")
            out.write(f"-- {len(view_files)} view(s) — CREATE TABLE scripts excluded\n")
            out.write(f"-- {'='*56}\n\n")

            for i, vf in enumerate(view_files):
                out.write(f"-- ── [{i+1}/{len(view_files)}] {vf.name} {'─'*max(0,44-len(vf.name))}\n\n")
                out.write(vf.read_text(encoding='utf-8').strip())
                out.write("\n\n")

        print(f"  📦 Combined {len(view_files)} view(s) → {combined_path.relative_to(combined_dir.parent)}")
        combined_count += 1

    if combined_count:
        print(f"\n  ✅ {combined_count} combined file(s) ready to run in BigQuery.")
...
def main():
    ...
    parser.add_argument(
        '--combined',
        default=COMBINED_FOLDER,
        help=f'Folder for combined view files (default: {COMBINED_FOLDER})'
    )
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f" Building combined view files (CREATE TABLE excluded)...")
        print(f"{'='*60}")
        write_combined_views(dst_dir, combined_dir)
    finally:
        if writer:
